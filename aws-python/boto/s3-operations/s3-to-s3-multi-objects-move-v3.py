...
def move_multiple_object():
    ...
    #create destination sub-directory by time at running process time
    current_date_time = time.strftime('%m%d%Y-%H%M%S')
    destination_object_dir=destination_prefix+current_date_time+"/"
    s3 = boto3.client('s3',
                      aws_access_key_id=aws_access_key_id,
                        aws_secret_access_key=aws_secret_access_key
    )
    s3.put_object(Bucket=destination_backet_name, Key=destination_object_dir)
    for object in source_bucket.objects.filter(Prefix=source_prefix):
        ...
        object_in_temp=object.key.replace(source_prefix, destination_object_dir)
