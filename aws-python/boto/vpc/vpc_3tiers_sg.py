...
def create_aws_vpc():
    ...
    infra_igw.create_tags(Tags=[{"Key": "Name", "Value": "infra-igw"},{"Key": "Project", "Value": "TrainingAndCertification"}])
    ...
    #create secuirty group for web server
    security_group_web_server=ec2Resource.create_security_group(GroupName="web-server",    
    Description="Allows necessary traffic only for web servers.",VpcId=infra_vpc.id)
    security_group_web_server.authorize_ingress(CidrIp="0.0.0.0/0",IpProtocol="tcp",FromPort=22,ToPort=22)
    security_group_web_server.authorize_ingress(CidrIp="0.0.0.0/0",IpProtocol="tcp",FromPort=80,ToPort=80)
    security_group_web_server.authorize_ingress(CidrIp="0.0.0.0/0",IpProtocol="tcp",FromPort=443,ToPort=443)
    security_group_web_server.create_tags(Tags=[{"Key": "Name", "Value": "webserver"}])
    print(f"Security group for web server {security_group_web_server.id} has been created.")
    #create secuirty group for rds(aloow ingress for mysql and postgres)
    security_group_rds=ec2Resource.create_security_group(GroupName="rds-group",    
    Description="Allows necessary traffic only for rds backend.",VpcId=infra_vpc.id)
    security_group_rds.authorize_ingress(CidrIp=vpc_cidr_block,IpProtocol="tcp",FromPort=3306,ToPort=3306)
    security_group_rds.authorize_ingress(CidrIp=vpc_cidr_block,IpProtocol="tcp",FromPort=5432,ToPort=5432)
    security_group_rds.create_tags(Tags=[{"Key": "Name", "Value": "rds"}])
    print(f"Security group for backend rds {security_group_rds.id} has been created.")
